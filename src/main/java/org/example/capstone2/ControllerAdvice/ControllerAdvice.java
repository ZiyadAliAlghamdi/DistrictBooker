...
public class ControllerAdvice {
    ...
    //java mail exceptions

    //connection issues handler
    @ExceptionHandler(value = MessagingException.class)
    public ResponseEntity<?> MessagingException(MessagingException messagingException){
        String message = messagingException.getMessage();
        return ResponseEntity.status(400).body(new ApiResponse(message));
    }

    //handles auth with mail server
    @ExceptionHandler(value = AuthenticationFailedException.class)
    public ResponseEntity<?> AuthenticationFailedException(AuthenticationFailedException authenticationFailedException){
        String message = authenticationFailedException.getMessage();
        return ResponseEntity.status(400).body(new ApiResponse(message));
    }

    //if the message doesn't send
    @ExceptionHandler(value = SendFailedException.class)
    public ResponseEntity<?> SendFailedException(SendFailedException sendFailedException){
        String message = sendFailedException.getMessage();
        return ResponseEntity.status(400).body(new ApiResponse(message));
    }

    //if the smtp server rejected the message
    @ExceptionHandler(value = SMTPAddressFailedException.class)
    public ResponseEntity<?> SMTPSendFailedException(SMTPAddressFailedException smtpAddressFailedException){
